...
public:
	CSimpleModule() {}
	virtual HRESULT AddCommonRGSReplacements(IRegistrarBase*) noexcept { return S_OK; }
...
launchedAppHwnd(NULL)
{
	...
	//{
	//	baseHost = "";
	//	LOG_ENTRY1("proxyHost: %s", baseHost.c_str());
	//}
}
	...
	try
	{
		...
		if (CookiesEngine::getCookiesFilePath().empty())
		{
			...
			try 
			{
				...
				std::wstring path = dir + L"rbxcsettings.rbx";
				CookiesEngine::setCookiesFilePath(path);
			}
			catch (...)
			{
...
void Bootstrapper::RegisterProtocolHandler(const std::wstring& protocolScheme, const std::wstring& exePath, const std::wstring& installKeyRegPath)
{
	...
	CreateKey(key->m_hKey, _T("shell\\open\\command"), value.c_str());
	...
	try
	{
		...
		if (queryInstalledVersion() == installVersion)
		{
			...
			else
			{
				...
				LOG_ENTRY1("checkingDirectory = %S", pathCheckingFor.c_str());
